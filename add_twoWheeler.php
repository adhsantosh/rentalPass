...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] == 0) {
        ...
        if ($uploadOk && move_uploaded_file($photo["tmp_name"], $targetFile)) {
            $stmt = $conn->prepare("INSERT INTO two_wheeler (name, model, price, available, photo) VALUES (?, ?, ?, ?, ?)");
            if ($stmt) {
                $stmt->bind_param("ssdsi", $name, $model, $price, $available, $targetFile);
                if ($stmt->execute()) {
                    $message = "<div class='alert alert-success'>Two-wheeler added successfully with photo!</div>";
                } else {
...
<div class="container mt-5">
    ...
    <form action="add_two_wheeler.php" method="POST" enctype="multipart/form-data">
        ...
        <button type="submit" class="btn btn-primary mt-3">Add Two-Wheeler</button>
    </form>
</div>
